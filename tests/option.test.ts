...
describe("input options", () => {
    it("should correctly label primes with user-specified numRounds", async () => {
        ...
        const results = await Promise.all(primes.map(p =>
            primalityTest(p, {numRounds: 8})
        ));
    ...
    it("should correctly label composite numbers with user-specified numRounds", async () => {
        ...
        const results = await Promise.all(composites.map(n =>
            primalityTest(n, {numRounds: 8})
        ));
        results.forEach((result) => {
    ...
    it("should correctly use a provided array of bases instead of random bases", async () => {
        ...
        const result1 = await primalityTest(341550071728321n, {bases: [2, 3, 5, 7, 11, 13, 17, 19]})
    ...
    it("should ignore the numRounds option when bases is specified", async () => {
        ...
        const result = await primalityTest(341550071728321n, {numRounds: 20, bases: [2]})
        expect(result.probablePrime).toBe(true)
    });
    ...
    it("should throw a RangeError when one or more provided bases is out of range", async ({expect}) => {
        await expect(primalityTest(113n, {bases: [1, 2, 3]})).rejects.toThrowError(RangeError)
        await expect(primalityTest(119n, {bases: [500]})).rejects.toThrowError(RangeError)
    });
    ...
    it("should not return a divisor if findDivisor=false", async () => {
        ...
        const results = await Promise.all(composites.map(n =>
            primalityTest(n, {findDivisor: false})
        ));
        results.forEach((result) => {
